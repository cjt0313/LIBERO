...
def summarize(output_dir):
    ...
    for suite, count in SUITES.items():
        for task_id in range(count):
            ...
            changes = list((task_dir / "state_changes").glob("episode_*/changes.json"))
            if not trace_path.exists() or len(videos) != 10 or len(changes) != 10:
                continue
...
def main():
    ...
    exporter = Path(__file__).with_name("export_state_change_frames.py")
    for suite, count in SUITES.items():
        for task_id in range(count):
            name = f"{suite}_{task_id:02}"
            task_dir = output_dir / name
            ...
            with (task_dir / "run.log").open("a") as log:
                ...
                subprocess.run([sys.executable, str(exporter), str(task_dir)],
